...
public class AmapGeocodingService : IAmapGeocodingService
{
    ...
    public async Task<AmapGeocodeResult?> GeocodeAsync(string query, string? cityFilter = null,
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            _logger.LogInformation("Calling AMap geocode for query {Query} (city filter: {CityFilter})", query,
                string.IsNullOrWhiteSpace(cityFilter) ? "(none)" : cityFilter);
            var response = await _httpClient.GetAsync(requestUri, cancellationToken);
            if (!response.IsSuccessStatusCode)
            {
                _logger.LogWarning("AMap geocoding request failed with status {StatusCode} for query {Query}",
            ...
            _logger.LogDebug(
                "AMap geocode success for query {Query}: {{lat={Latitude}, lng={Longitude}, place={Place}}}", query,
                latitude, longitude, first.FormattedAddress ?? placeName);
    ...
    private string BuildRequestUri(string query, string? cityFilter)
    {
        ...
        if (!string.IsNullOrWhiteSpace(cityFilter)) builder.Append("&city=").Append(Uri.EscapeDataString(cityFilter));
    ...
    private static string? ExtractFlexibleString(JsonElement element)
    {
        return element.ValueKind switch
        {
            ...
            JsonValueKind.Object when element.TryGetProperty("name", out var nameElement) => ExtractFlexibleString(
                nameElement),
            _ => null
        };
    ...
    private sealed class AmapGeocodeResponse
    {
        ...
        [JsonPropertyName("geocodes")] public List<AmapGeocodeItem> Geocodes { get; set; } = new();
    }
    ...
    private sealed class AmapGeocodeItem
    {
        ...
        [JsonPropertyName("building")] public AmapNameValue? Building { get; set; }

        [JsonPropertyName("neighborhood")] public AmapNameValue? Neighborhood { get; set; }

        [JsonPropertyName("city")] public JsonElement CityElement { get; set; }

        [JsonIgnore] public string? City => ExtractFlexibleString(CityElement);

        [JsonPropertyName("province")] public JsonElement ProvinceElement { get; set; }

        [JsonIgnore] public string? Province => ExtractFlexibleString(ProvinceElement);

        [JsonPropertyName("township")] public JsonElement TownshipElement { get; set; }

        [JsonIgnore] public string? Township => ExtractFlexibleString(TownshipElement);

        [JsonPropertyName("pois")] public List<AmapPoi>? Pois { get; set; }

        [JsonPropertyName("name")] public string? POIName => Pois?.FirstOrDefault()?.Name;
    }

    private sealed class AmapNameValue
    {
        [JsonPropertyName("name")] public JsonElement NameElement { get; set; }

        [JsonIgnore] public string? Name => ExtractFlexibleString(NameElement);
    }
