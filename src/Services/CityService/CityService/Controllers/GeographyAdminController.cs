...
public class GeographyAdminController : ControllerBase
{
    ...
    public GeographyAdminController(
        ...
        ILogger<GeographyAdminController> logger)
    {
        _seeder = seeder;
        _logger = logger;
    }
    ...
    public async Task<IActionResult> SeedChinaDefault()
    {
        ...
        {
            // 使用用户提供的完整中国省市数据
            var chinaData = GetChinaProvincesCitiesData();
  ...
  { ""province"": ""香港特别行政区"", ""cities"": [""香港岛"", ""九龙"", ""新界"", ""离岛""] },
  { ""province"": ""澳门特别行政区"", ""cities"": [""澳门半岛"", ""氹仔"", ""路环"", ""路氹城""] }
]";

        return JsonSerializer.Deserialize<List<ProvinceData>>(jsonData) ?? new List<ProvinceData>();
    }
}
